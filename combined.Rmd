...
The predictors selected are: gender, ethnic_group, parent_educ, lunch_type, test_prep, practice_sport, and wkly_study_hours. 
...
cv_df_rownames = cv_df |> row.names()
cv_df_colnames = cv_df |> colnames()

cv_df = cv_df |> 
  as.data.frame() |>
  cbind(cv_df_rownames) |>
  rename(model_name = cv_df_rownames) |>
  ...
  mutate(
    ...
    test = map(model_name, \(i) str_extract(i, "(?<=_)[:alpha:]+")),
    manual = map(model_name, \(i) str_extract(i, "(?<=_)[:alpha:]+(?=_fit)"))
  ) |>
  dplyr::select(-model_name) |>
  relocate(subject, test, manual, terms)
...
```{r}
cv_report_df = cv_df |>
  dplyr::select(subject, test, manual, terms) |>
  mutate(model = map(terms, \(formula) train(as.formula(formula),
                   data = step_df,
                   trControl = train,
                   method = 'lm',
                   na.action = na.pass)),
         final_model = map(model, \(i) i$finalModel),
         MSE = map(final_model, \(i) mean(i$residuals^2)))
```
