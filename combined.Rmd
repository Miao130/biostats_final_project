...
```{r}
math_models_report_df = cv_lecture_df |>
  filter(subject == "math")

math_models_report_df |>
  dplyr::select(method, MSE) |>
  knitr::kable()

math_models_report_df = math_models_report_df |>
  filter(MSE == min(math_models_report_df$MSE |> unlist()))

# Only one value
math_best_fit = math_models_report_df$model[[1]]
```
The model with the best MSE for Math is `r paste(math_models_report_df$terms)`, which uses `r paste(math_models_report_df$method)` as a method of approach.
...
### Method from the lecture codes
```{r}
reading_models_report_df = cv_lecture_df |>
  filter(subject == "reading")

reading_models_report_df |>
  dplyr::select(method, MSE) |>
  knitr::kable()

reading_models_report_df = reading_models_report_df |>
  filter(MSE == min(reading_models_report_df$MSE |> unlist()))

# Only one value
reading_best_fit = reading_models_report_df$model[[1]]
```
The model with the best MSE for Reading is `r paste(reading_models_report_df$terms)`, which uses `r paste(reading_models_report_df$method)` as a method of approach.
...
### Method from the lecture codes
```{r}
writing_models_report_df = cv_lecture_df |>
  filter(subject == "writing")

writing_models_report_df |>
  dplyr::select(method, MSE) |>
  knitr::kable()

writing_models_report_df = writing_models_report_df |>
  filter(MSE == min(writing_models_report_df$MSE |> unlist()))

# Only one value
writing_best_fit = writing_models_report_df$model[[1]]
```
The model with the best MSE for Writing is `r paste(writing_models_report_df$terms)`, which uses `r paste(writing_models_report_df$method)` as a method of approach.
