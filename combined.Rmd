...
```
`gender`, `test_prep` -> `wkly_study_hours`
`test_prep` -> `nr_siblings`
...
Hence, `gender`, `test_prep`, `parent_educ` could be protential confounder for `wkly_study_hours`
...
lm(writing_score ~ gender * test_prep * wkly_study_hours * parent_educ + ethnic_group + lunch_type, data = step_df) |>
  broom::tidy() |> 
  knitr::kable(caption = "Writing: with Number of Siblings")
```
