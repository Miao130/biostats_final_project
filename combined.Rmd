...
math_mlr_fit = model_math
reading_mlr_fit = model_read
writing_mlr_fit = model_write
```
...
math_criteria_fit = results$which[7,]
math_criteria_fit|>print()
```
...
reading_criteria_fit = results$which[5,]
reading_criteria_fit |>print()
```
...
writing_criteria_fit = results$which[5,]
writing_criteria_fit|>print()
```
...
coef_df_rownames = coef_df |> row.names()

coef_df = coef_df |>
  as.data.frame() |>
  cbind(coef_df_rownames) |>
  rename(model_name = coef_df_rownames)
...
cv_df = cv_df |> 
  ...
  mutate(across(-c(model_name, terms), \(i) ifelse(is.na(i), FALSE, TRUE))) |>
  bind_rows(coef_df) |>
  mutate(
    across(-c(model_name, terms), \(i) ifelse(i, "X", NA)),
...
```{r}
coef_names = cv_df |> dplyr::select(-c(subject, test, manual, "(Intercept)", terms)) |> colnames()
                             
k = cv_df |> 
  nest(-c(subject, test, manual, "(Intercept)", terms)) |>
  mutate(data = map(data, as.list),
         data = cbind(data, colnames))
```
```{r}
helper = function(li, names_li){
  toReturn = ""
