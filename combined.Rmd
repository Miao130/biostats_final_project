...
```
# Cross Validation
Here are the summary of all the models included in this project

```{r}
# Clean out the variables used in stepwise analysis
var_names = step_df |> dplyr::select(!ends_with("score"))  |> colnames()

math_mlr_fit = lm(math_score ~ gender + ethnic_group + parent_educ + lunch_type + test_prep + parent_marital_status + nr_siblings + wkly_study_hours, data = step_df)

reading_mlr_fit = lm(reading_score ~ gender + ethnic_group + parent_educ + lunch_type + test_prep + parent_marital_status + wkly_study_hours, data = step_df)

writing_mlr_fit = lm(writing_score ~ gender + ethnic_group + parent_educ + lunch_type + test_prep + parent_marital_status + wkly_study_hours, data = step_df)
...
cv_report_df_rownames = cv_report_df |> row.names()
cv_report_df_colnames = cv_report_df |> colnames()

cv_report_df = cv_report_df |> 
  as.data.frame() |>
  cbind(cv_report_df_rownames) |>
  rename(model_name = cv_report_df_rownames) |>
  ...
  mutate(
    ...
    method = map(model_name, \(i) str_extract(i, "(?<=_)[:alpha:]+")),
    manual = map(model_name, \(i) str_extract(i, "(?<=_)[:alpha:]+(?=_fit)"))
  ) |>
  dplyr::select(-model_name) |>
  relocate(subject,method, manual, terms)
...
cv_lecture_df = cv_report_df |>
  dplyr::select(subject, method, manual, terms) |>
  mutate(model = map(terms, \(formula) train(as.formula(formula),
                   data = step_df,
                   trControl = train,
                   method = 'lm',
                   na.action = na.pass)),
         model = map(model, \(i) i$finalModel[[1]]))
```

## Method from Data Science
```{r}
set.seed(1)
cv_ds_df = 
  ...
  mutate(
    method = map2(method, manual,  \(i,j) ifelse(j == "manual" | j == "func", paste0(i, "_", j), paste0(i)))) |>
  dplyr::select(-manual) |>
  mutate(
...
```{r}
math_cv_report_df = cv_lecture_df |>
  filter(subject == "math")

math_cv_report_df = math_cv_report_df |>
  filter(MSE == min(math_cv_report_df$MSE |> unlist()))

# Only one value
math_best_fit = math_cv_report_df$model[[1]]
``````
The model with the best MSE for Math is `r paste(math_cv_report_df$terms)`, which uses `r paste(math_cv_report_df$test)` as a method of approach.
...
cv_ds_df |> 
  ...
  knitr::kable()
```
We noticed that the the best model is the one that uses forward elimination with one line code and backward elimination with one line code. The model is `r cv_report_df |> filter(subject == "math", method == "forward", manual == "func") |> pull(terms) |> paste()`
...
### Method from the lecture codes
```{r}
reading_cv_report_df = cv_lecture_df |>
  filter(subject == "reading")

reading_cv_report_df = reading_cv_report_df |>
  filter(MSE == min(reading_cv_report_df$MSE |> unlist()))

# Only one value
reading_best_fit = reading_cv_report_df$model[[1]]
```
The model with the best MSE for Reading is `r paste(reading_cv_report_df$terms)`, which uses `r paste(reading_cv_report_df$test)` as a method of approach.
...
cv_ds_df |> 
  ...
  knitr::kable()
```
We noticed that the the best model is the model that is picked by forward and backward elimination method and criterion based approach. The model is `r cv_report_df |> filter(subject == "reading", method == "forward", manual == "func") |> pull(terms) |> paste()`

## Cross Validation - Writing
### Method from the lecture codes
```{r}
writing_cv_report_df = cv_lecture_df |>
  filter(subject == "writing")

writing_cv_report_df |>
  

writing_cv_report_df = writing_cv_report_df |>
  filter(MSE == min(writing_cv_report_df$MSE |> unlist()))

# Only one value
writing_best_fit = writing_cv_report_df$model[[1]]
```
The model with the best MSE for Writing is `r paste(writing_cv_report_df$terms)`, which uses `r paste(writing_cv_report_df$test)` as a method of approach.
...
cv_ds_df |> 
  ...
  knitr::kable()
```
We noticed that the the best model is the one that uses forward elimination with one line code and backward elimination with one line code. The model is `r cv_report_df |> filter(subject == "writing", method == "forward", manual == "func") |> pull(terms) |> paste()`

# Effect Modifier
To check over effect modifier, we will take the methods logically. 
Effect modification occurs when
```{r}
